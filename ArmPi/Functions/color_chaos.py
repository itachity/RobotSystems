...
ACQUIRE_SEC = 0.9
MIN_SAMPLES_PER_COLOR = 4

# Random off-board drop zones for "yeet" mode
YEET_ZONES = [
    (-4.0, 16.0),
    (4.0, 16.0),
    (9.0, 12.0),
...
# Attack/drop heights
ATTACK_APPROACH_Z = 10.0
ATTACK_RELEASE_Z = 3.5
YEET_Z = 13.0
...
def detect_blocks(frame_bgr):
    """
    Detect at most one block per target color.
    Returns:
        frame_rs: resized frame used by perception
...
def average_detections(samples_by_color, min_samples=MIN_SAMPLES_PER_COLOR):
    """
    Average detections over a short time window to reduce jitter.
    One stable detection per color.
    """
    stable = []
...
def draw_scene(frame_rs, blocks, status_text="Scanning...", status_color=(0, 255, 255)):
    out = frame_rs.copy()
    h, w = out.shape[:2]
    ...
    cv2.putText(out, status_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, status_color, 2)
    return out


def show_status(frame_rs, blocks, status_text, status_color=(0, 255, 255), hold_ms=300):
    vis = draw_scene(frame_rs, blocks, status_text=status_text, status_color=status_color)
    cv2.imshow("Color Chaos", vis)
    cv2.waitKey(max(1, hold_ms))


# ============================================================
# Dance / action helpers
# ============================================================

def dance_with_block(arm, cx, cy, z=12.0, radius=6.9, loops=1):
    """
    Dynamic, waypoint-based dance while holding a block.
    Uses:
      - swagger
      - bounce
      - diagonal cross-step
      - wrist flicks
    This is much smoother than tiny circular IK steps.
    """
...
def chaos_attack(arm, attacker, target, frame_rs, stable_blocks):
    """
    Pick attacker, then show 'Attacking ...', dance, and drop onto target.
    """
    ...
    # Only show this AFTER successful pickup
    show_status(
        frame_rs,
        stable_blocks,
        f"Attacking {target.color} block",
    ...
    arm.move_xyz(target.world_x, target.world_y, ATTACK_APPROACH_Z)
    time.sleep(0.2)
    arm.move_xyz(target.world_x, target.world_y, ATTACK_RELEASE_Z, t_ms=400)
    time.sleep(0.15)
    arm.open_gripper()
    time.sleep(0.35)
...
def chaos_yeet(arm, attacker, frame_rs, stable_blocks):
    """
    Pick attacker, then show 'Yeeting ...', dance, and drop in a random yeet zone.
    """
    arm.set_led(attacker.color)
    arm.beep(0.1)
    ...
    show_status(
        frame_rs,
        stable_blocks,
        f"Yeeting {attacker.color} block",
    ...
    arm.move_xyz(yeet_x, yeet_y, YEET_Z)
    time.sleep(0.2)
    arm.open_gripper()
    time.sleep(0.3)
    arm.home()
...
def main():
    # IMPORTANT:
    # Replace gripper_open / gripper_close with your calibrated values.
    # Tune z_pick / y_offset as needed on your real arm.
    cfg = MotionConfig(
        gripper_open=220,
        gripper_close=540,
        z_pick=0.3,
    ...
    try:
        while True:
            ...
            vis = draw_scene(frame_rs, blocks, status_text="Scanning for chaos...", status_color=(0, 255, 255))
            cv2.imshow("Color Chaos", vis)
            ...
            if (time.time() - acquire_t0) < ACQUIRE_SEC:
                continue

            stable_blocks = average_detections(samples_by_color, min_samples=MIN_SAMPLES_PER_COLOR)

            # reset acquisition window
            samples_by_color = defaultdict(list)
            acquire_t0 = time.time()
            ...
            if len(stable_blocks) == 1:
                ...
                show_status(
                    frame_rs,
                    stable_blocks,
                    f"Picking up {attacker.color} block",
                    status_color=(0, 255, 0),
                    hold_ms=350,
                )
                chaos_yeet(arm, attacker, frame_rs, stable_blocks)
                time.sleep(0.6)
                continue
            ...
            chaos_attack(arm, attacker, target, frame_rs, stable_blocks)
            time.sleep(0.6)
